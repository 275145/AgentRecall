...
3. 如果稳定开始时间缺失，同日标题先保留 `MM-DD`；只有标签仍重复时才追加任务目录 basename；
...
项目排序在现有“本地环境优先、最近活动优先、基础名称”之后，依次比较 `labelSuffix ?? ""`、`path` 和 `environmentId`，确保所有主排序键相同时仍得到确定顺序。
