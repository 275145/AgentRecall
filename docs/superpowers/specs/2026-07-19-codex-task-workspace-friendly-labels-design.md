...
4. 如果 basename 后可见标签仍相同，从最近父级向外按相同相对深度查找组内唯一的非空路径片段并追加；没有可用唯一片段时追加原始项目路径；若刻意构造的标题在追加原始路径后仍与另一个结构不同的标签显示成相同文本，再追加由 `environmentId + path` 编码出的稳定身份后缀。
...
消歧按以下顺序执行：先保留“同一物理路径出现在多个环境”时的环境名称后缀，再在每个环境内部对任务标题执行日期、时间、basename、唯一父路径片段、原始路径和最终稳定身份消歧。日期/时间阶段之后的碰撞键必须与 renderer 一致，直接使用实际可见的 `base + " · " + suffix`，不能用不可见分隔符保存基础名称与后缀的结构边界。因此标题 `Foo · 07-19` 与基础标题 `Foo` 加后缀 `07-19` 会进入同一碰撞组。

有标题任务的可见基础名称是 `summary.label`。未命名任务同时生成 `Untitled session` 和 `未命名会话` 两个可见变体；任一变体发生碰撞，该任务都参与本轮消歧。英文、中文碰撞组通过共享的未命名任务合并为同一个连通组，并按环境身份与原始路径的严格顺序处理，避免遍历顺序影响结果。

每次追加 basename 后都重新计算可见碰撞组；追加最短唯一父片段或原始路径后再次计算；追加最终稳定身份后继续重算，直到两个语言下都没有重复。`taskBasenameApplied` 保证 basename 最多追加一次，已有后缀的顺序保持不变。最终身份使用唯一项目身份的确定性编码；每轮为同一碰撞组产生不同尾段，已处理的碰撞对不会再次相等，而项目对数量有限，因此循环必然终止。
...
- 标题中的 ` · ` 与生成后缀形成同一可见文本时会继续消歧，`custom_title` 使用相同规则。
- 有标题任务分别与未命名任务的英文、中文渲染发生碰撞时都能进入同一确定性消歧波次。
- basename、父片段/原始路径及最终身份阶段之后都重新检查两种语言的实际可见标签，直到全部唯一。
