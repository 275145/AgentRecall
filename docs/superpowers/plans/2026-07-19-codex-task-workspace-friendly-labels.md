...
在 `src/core/store/sessions.ts` 保留日期/时间阶段的同标题统计，并把其后的碰撞处理实现为与 renderer 同契约的可见字符串分组。实现结构如下：
...
function visibleTaskCollisionGroups(summaries: ProjectSummaryDraft[]): ProjectSummaryDraft[][] {
  // 对同一 environment 中共享任一完整可见 variant 的 summary 做 union-find。
  // untitled 的英文/中文 variant 可以把两个直接碰撞组合并为一个连通组。
  // 返回前按 environmentId + path 的严格比较器排序组和组成员。
}

// 1. 日期/时间后按 visibleTaskCollisionGroups() 追加一次 basename。
// 2. 重新分组，优先追加最短唯一父片段，否则追加 raw path。
// 3. 再次分组，对残余组追加 environmentId + path 的稳定身份编码。
// 4. identity 阶段循环重验，直到英文、中文可见结果都没有碰撞。
```
